...
for sampler in ['graphsaint']:
    for alg in ['sage']:
        ...
        for bs in batch_sizes[f'{data}_{sampler}_{alg}']:
            print("batch_size", bs)
            st = time.time()
            cmd = f"python -u {file_path} --epochs 60 --runs 1 --device {device_id} --batch_size {bs} 1>log/{data}_{sampler}_{alg}_{bs}.out 2>&1"
            os.system(cmd)
            ed = time.time()
            os.system("date")
            print(f"use_time: {ed - st}")